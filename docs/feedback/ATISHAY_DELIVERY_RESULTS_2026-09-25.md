...
| D — hands-free voice/images | PARTIAL | Deterministic live-voice harness, installed-ASR preview/final and reconnect tests, per-image perception regressions, and `image_staging_check.cjs` | The browser has opt-in provisional ASR and automatic final capture. A resumed-speech revision invalidates an in-flight old preview. A dropped PNG now uploads the same file shown in its thumbnail; picker behavior also passes. Distinct accepted image IDs no longer suppress one another in owned perception; a full image queue rejects new work explicitly. | Physical-mic timing, semantic endpoint quality, committed-effect closure, ordered multi-image registry/field selection and actual vision remain open. D2/D3 controller hooks and D4 registry/source guards require Mridul; receipt-based browser attachment history remains Atishay-owned. |
...
| Gate 4 — regression | PASS for current code; acceptance still partial | Python 3.11.15: `python -m pytest -q` **1384 passed, 6 skipped, 3 xfailed, 2 dependency warnings** (111.29 s final run); `ruff check .`, six Node checks, and inline browser-script syntax passed. Four explicit installed-ASR opt-in tests: **4 passed** (67.91 s final run). | Owned perception preserves separate image IDs/failures and rejects queue overflow; configured browser/controller checks cover repeated-quantity mock effects and disconnect during a gated mock write; installed Faster Whisper decodes generated WAV preview/final and reconnect cases; picker/drop staging matches the actual uploaded file. | An earlier full run failed a one-second image-provider-start wait with `MemoryError`; it passed alone and on subsequent full runs, root cause unproven. Two D1 xfails and the D4 conflicting-frame xfail remain deliverable failures; physical-mic timing and configured reasoning remain unverified. |
