...
| D — hands-free voice/images | PARTIAL | Deterministic live-voice harness, installed-ASR preview/final and reconnect tests, per-image perception regressions, `image_staging_check.cjs` and `attachment_history_check.cjs` | The browser has opt-in provisional ASR and automatic final capture, with a conservative hold for an explicit unfinished correction. A dropped PNG uploads the thumbnailed file. Separately receipted PNGs now retain source-bound thumbnails and processing/failure status in the browser session; a later text-only answer cannot reuse a staged image preview. The browser refuses a ninth image explicitly. Distinct image IDs remain separate in owned perception. | Physical-mic timing, general semantic endpoint quality, committed-effect closure, authoritative Image 1/2/3 ordinals, shared multi-image registry/field selection and actual vision remain open. The browser list is not agent reasoning evidence. D2/D3 controller hooks and D4 registry/source guards require Mridul. |
...
| I01 | BLOCKED at controller | Owned perception and demo-wrapper tests retain separately identified images. `attachment_history_check.cjs` and `answer_correlation_check.cjs` now retain browser thumbnails by accepted source/event and reject duplicate/old-session receipts. No Image 1/2/3 label is assigned without a server ordinal; D4 registry and shuffled-completion conformance remain absent in Mridul's controller. |
...
| I04 | BLOCKED at controller | Owned perception surfaces distinct-image failure and rejects a ninth pending image. The browser now retains a failed accepted image as unavailable, refuses a ninth local session attachment before upload, and clears object URLs on a fresh session. D4 bounded authoritative registry, stable ordinals and shared failure lifecycle remain absent in Mridul's controller. |
