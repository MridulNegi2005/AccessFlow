...
| I04 | BLOCKED at controller | Owned perception surfaces distinct-image failure and rejects a ninth pending image. The browser retains a failed accepted image as unavailable, refuses a ninth local session attachment before upload, and clears object URLs on a fresh session. A later tool/task error no longer marks an observed image as failed; `answer_correlation_check.cjs` and `attachment_history_check.cjs` cover this. D4 bounded authoritative registry, stable ordinals and shared failure lifecycle remain absent in Mridul's controller. |
