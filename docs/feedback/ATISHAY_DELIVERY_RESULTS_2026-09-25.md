...
| D — hands-free voice/images | PARTIAL | Deterministic live-voice browser harness and fake-ASR WebSocket preview/final tests | The browser now offers opt-in live capture when configured process ASR is available, provisional speech preview, quiet-time completion, and End session resource cleanup. No synthetic recording is called live microphone evidence. | Physical-mic timing and interruption, semantic endpoint quality, committed-effect closure, and ordered multi-image history remain open. D2/D3 controller hooks and D4 registry/source guards require Mridul; browser capture/display belongs to Atishay. |
