...
| V05 | PARTIAL | `live_voice_check.cjs` sends provisional previews before automatic final, now including later speech after three previews. Defaults are 2.2 s fluent quiet completion, 5.5 s unfinished/no-decode failure, 1 s preview spacing, 12 previews and a 60 s turn cap; these are validated constructor options, not measured ideal values. Exhaustion fails without final audio. The earlier installed-ASR WebSocket test observed a generated-WAV preview before final admission and zero mock planner calls until final, but the later low-memory opt-in rerun failed. No physical-mic or real-reasoner action proof. |
