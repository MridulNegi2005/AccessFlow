...
- The demo now exposes an optional loopback Ollama JSON reasoner through `ACCESSFLOW_DEMO_OLLAMA_REASONER_MODEL`; requests are bounded, returned plans are strictly parsed as `PlanProposal`, and provider failures remain recoverable through the Agent. The mock reasoner remains the default, and no live reasoning quality claim is made without a running local service.
...
- Final verification is 250 tests passed with 4 strict expected failures, including 103 passing demo
