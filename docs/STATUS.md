...
- The demo now exposes an optional loopback Ollama JSON reasoner through `ACCESSFLOW_DEMO_OLLAMA_REASONER_MODEL`; requests are bounded, returned plans are strictly parsed as `PlanProposal`, provider failures remain recoverable through the Agent, and a WebSocket regression verifies both frame evidence and a later spoken request reach the reasoner. The browser status now labels perception and reasoning backends together. The mock reasoner remains the default, and no live reasoning quality claim is made without a running local service.
...
- Final verification is 251 tests passed with 4 strict expected failures, including 104 passing demo
