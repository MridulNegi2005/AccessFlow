...
Tests run and results: `uv run --python 3.12 --extra dev pytest -q` — 77 passed; perception run — 43
passed; demo run — 18 passed; held-out fixture check — 1 passed; Ruff clean for owned paths.
...
Known failures: Default 3.11 uv target link is broken on this machine; manual browser/device smoke proof, human speech accuracy and endpoint quality, live vision quality, image-only controller integration, hosted backend, completed user feedback session and demo recording are not implemented.
